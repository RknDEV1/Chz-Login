...
- (void)loginWithKey:(NSString *)key success:(CHZAuthSuccess)success failure:(CHZAuthFailure)failure {
    ...
    // O callback `success` do SDK é a fonte autoritativa: a biblioteca só deve
    // chamá-lo quando a key tiver sido aceita pelo servidor. Não interpretamos
    // um JSON genérico nem exigimos getters que podem ser preenchidos depois.
    apiclient_dict_callback onSuccess = ^(__unused const char *json) {
